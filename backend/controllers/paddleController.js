...
const createTransactionForCheckout = asyncHandler(async (req, res) => {
    ...
    console.log(`[Step 1] Initiating checkout for local Product ID: ${productId}`);
    ...
    if (!product.paddlePriceId) {
        res.status(500);
        throw new Error('This product is not configured for payments on the server.');
    }
    ...
    try {
        console.log(`[Step 2] Verifying Paddle resources for Price ID: ${product.paddlePriceId}`);
        const priceResponse = await paddleApi.get(`/prices/${product.paddlePriceId}`);
        if (priceResponse.data.data.status !== 'active') {
            throw new Error('This product has an inactive price and cannot be purchased.');
        }
        console.log(`✅ Paddle price is active.`);
    } catch (axiosError) {
        console.error("❌ PADDLE PRE-FLIGHT CHECK FAILED:", axiosError.response?.data || axiosError.message);
        throw new Error("Could not verify product details with our payment provider.");
    }
    ...
    try {
        const payload = {
            ...
            customer: user.paddleCustomerId ? { id: user.paddleCustomerId } : { email: user.email },
            custom_data: {
        ...
        if (!transactionId) throw new Error("Could not initialize the payment session.");
    ...
    try {
        const payload = {
            items: [{ price_id: product.paddlePriceId, quantity: parseInt(quantity, 10) || 1 }],
            customer: { email: user.email },
            customData: { userId: user._id.toString(), productId: product._id.toString(), quantity: parseInt(quantity, 10) || 1 }
        };

        console.log("[Step 3] Sending final payload to /transactions:", JSON.stringify(payload, null, 2));

        const response = await paddleApi.post('/transactions', payload);
        const transactionId = response.data?.data?.id;

        if (!transactionId) {
            throw new Error("Payment provider created a transaction but did not return an ID.");
        }
        ...
        console.error("❌ FINAL TRANSACTION ERROR:", axiosError.response?.data?.error || axiosError.message);
        const errorMessage = axiosError.response?.data?.error?.detail || "An unexpected final error occurred during checkout creation.";
...
const handlePaddleWebhook = asyncHandler(async (req, res) => {
    ...
    if (!signature || !webhookSecret || !Buffer.isBuffer(rawRequestBody)) {
        return res.status(400).send('Webhook validation failed: required data missing.');
    }
    ...
    try {
        ...
        if (event.eventType === 'transaction.completed') {
            ...
            if (!customData?.userId || !customData?.productId || !customData?.quantity) {
                return res.status(200).send('Acknowledged but cannot process without custom data.');
            }

            const { userId, productId, quantity } = customData;
            const { id: paddleTransactionId, customer_id: paddleCustomerId, details } = transactionData;
            ...
            const updatedProduct = await Product.findOneAndUpdate(
                ...
                { $inc: { amountLeft: -quantity } }, { new: true }
            );
            if (!updatedProduct) {
                console.error(`Webhook CRITICAL: Stock update failed for product ${productId}. REFUND MAY BE REQUIRED.`);
            ...
            if (purchasePriceInCents === undefined || !currencyCode) {
                console.error(`Webhook CRITICAL: Transaction ${paddleTransactionId} is missing pricing information.`);
                return res.status(200).send('Acknowledged but cannot process without pricing information.');
            }
            ...
            console.log(`- User ${userId} paddleCustomerId updated to ${paddleCustomerId}.`);
        }

        // Always acknowledge receipt to Paddle
        res.sendStatus(200);
...
const getLivePrice = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        console.error(`❌ PADDLE PRICE FETCH ERROR for ID ${priceId}:`, axiosError.response?.data);
...
const previewPrice = asyncHandler(async (req, res) => {
    ...
    if (!priceId) {
        res.status(400); throw new Error("A Price ID is required.");
    }
    ...
    try {
        ...
        const errorMessage = axiosError.response?.data?.error?.detail || "Could not retrieve live pricing.";
        res.status(500).json({ message: errorMessage });
    }
});
