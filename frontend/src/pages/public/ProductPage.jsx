...
const ProductPage = () => {
    ...
    const { userInfo, addToCart } = useAuth();
    ...
    const handleAddToCart = async () => {
        ...
        try {
            ...
            // Optionally, show a success toast/popup message here
    ...
    const handleCheckoutComplete = useCallback(
        (transactionData) => {
            ...
            if (transactionId) {
                console.log(
                    `Purchase complete! Transaction: ${transactionId}. Redirecting to success page...`
                );
                ...
                console.error(
                    "Checkout completed, but no transaction_id was found in the event data.",
                    transactionData
                );
            ...
            const fetchLivePrice = async () => {
                try {
                    const { data } = await API.get(
                        `/api/paddle/price/${product.paddlePriceId}`
                    );
                    setLivePrice(data);
                } catch (error) {
                    console.error(
                        "Could not fetch live price, falling back to DB price.",
                        error
                    );
                }
            };
    ...
    const handleQuantityChange = (amount) => {
        setQuantity((prev) =>
            Math.max(1, Math.min(prev + amount, product?.amountLeft || 1))
        );
    };
    ...
    const handleBuyNow = async () => {
        ...
        if (isCreatingCheckout || !isPaddleReady) {
            if (!isPaddleReady)
                setCheckoutError("Payment system is not loaded yet. Please wait.");
            return;
        }
        ...
        try {
            ...
            const { data } = await API.post(`/api/products/${id}/checkout`, {
                quantity,
            });
                    ...
                    window.Paddle.Checkout.open({
                        ...
                        eventCallback: (eventData) => {
                            if (eventData.name === "checkout.completed") {
                                const transactionId =
                                    eventData.data?.id || eventData.data?.transaction_id;
                                console.log(
                                    `Purchase complete! ID: ${transactionId}. Redirecting...`
                                );
    ...
    if (pageError)
        return (
            <div className="min-h-screen flex flex-col items-center justify-center text-center text-red-600 bg-red-50 p-8">
                <FontAwesomeIcon
                    icon={faExclamationTriangle}
                    className="text-4xl mb-4"
                />
                <h2 className="text-2xl font-bold">{pageError}</h2>
                <Link to="/shop" className="mt-6 text-sm font-semibold underline">
                    Back to Shop
                </Link>
            </div>
        );
    ...
    const displayPrice = livePrice
        ? (livePrice.amount / 100).toFixed(2)
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
                    ...
                    <div className="product-anim lg:col-start-2">
                        <div className="space-y-4">
                            ...
                            <h1 className="text-4xl lg:text-5xl font-extrabold font-serif text-primary-text leading-tight">
                                {name}
                            </h1>
                            <div>
                                <p className="text-4xl font-serif text-primary-text pt-2 h-14 flex items-center">
                                    ...
                                    ) : !livePrice && !isOutOfStock ? (
                                        <span className="text-2xl text-gray-400 animate-pulse">
                                            Verifying price...
                                        </span>
                                    ) : (
                                        `${displayPrice}`
                                    )}
                                    <span className="text-lg text-secondary-text ml-2">
                                        {!userInfo ? "" : displayCurrency}
                                    </span>
                                </p>
                                <p className="flex items-center gap-2 text-base text-secondary-text mt-2">
                                    <FontAwesomeIcon icon={faGlobe} />
                                    <span>
                                        Final price shown in your local currency at checkout.
                                    </span>
                                </p>
                            </div>
                            <div className="prose text-secondary-text max-w-none leading-relaxed pt-2">
                                <p>{description}</p>
                            </div>
                        </div>
                        ...
                        <div className="mt-8 pt-8 border-t border-gray-200">
                            {isOutOfStock ? (
                                <div className="w-full text-center p-4 bg-gray-100 text-gray-500 font-bold rounded-lg flex items-center justify-center gap-3">
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center border border-gray-300 rounded-md overflow-hidden">
                                            <button
                                                ...
                                                disabled={quantity <= 1}
                                                className="w-10 h-10 text-lg text-gray-600 hover:bg-gray-100 disabled:opacity-50 transition-colors"
                                                aria-label="Decrease quantity"
                                            >
                                                <FontAwesomeIcon icon={faMinus} size="xs" />
                                            </button>
                                            <span className="w-12 h-10 text-center font-bold text-lg flex items-center justify-center border-x">
                                                {quantity}
                                            </span>
                                            <button
                                                onClick={() => handleQuantityChange(1)}
                                                disabled={quantity >= amountLeft}
                                                className="w-10 h-10 text-lg text-gray-600 hover:bg-gray-100 disabled:opacity-50 transition-colors"
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-3">
                                        <button
                                            ...
                                            className="w-full bg-primary-text text-white font-bold py-3 px-6 rounded-lg text-lg flex items-center justify-center gap-3 hover:bg-opacity-80 transition-all disabled:bg-gray-400 disabled:cursor-wait"
                                        >
                                            {isAddingToCart ? (
                                        ...
                                        <button
                                            ...
                                            disabled={isCreatingCheckout || !isPaddleReady}
                                        ...
                                        >
                                            {isCreatingCheckout || !isPaddleReady ? (
                                                <FontAwesomeIcon icon={faSpinner} spin />
                                            ) : (
                                        ...
                                        {amountLeft < 10 && (
                                            <p className="text-center text-sm text-red-600 font-semibold animate-pulse">
                                                Only {amountLeft} left in stock - order soon!
                                            </p>
                                        )}
                                    </div>
                                </div>
                            ...
                            {checkoutError && (
                                <div className="text-center mt-4 p-3 bg-red-50 text-red-600 text-sm font-semibold rounded-lg flex items-center justify-center gap-2">
